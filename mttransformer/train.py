...
def evaluation(model, datasets, data_list, task_defs, output_dir='checkpoints', epoch=0, n_updates=-1, with_label=False, tensorboard=None, glue_format_on=False, test_on=False, device=None, logger=None):
    ...
    for idx, dataset in enumerate(datasets):
        ...
        if test_data is not None:
            with torch.no_grad():
                #la funzione eval_model in mt-dnn/inference.py chiama model.predict
                #in cui il mtdnn chiama sanbert per tirare fuori lo score
                #e tramite questo, utilizza un'altra funzione, per tirare fuori score, predict e label
                #e passarle alla funzione eval_model, la quale prende questi dati
                #e calcola con la funzione calc_metrics il risultato con la metrica scelta
