...
def train_config(parser):
    ...
    parser.add_argument('--apply_scheduler', action='store_true') ##AGGIUNTA GAN
...
def evaluation(model, datasets, data_list, task_defs, output_dir='checkpoints', epoch=0, n_updates=-1, with_label=False, tensorboard=None, glue_format_on=False, test_on=False, device=None, logger=None, finetuning=False, task=0):
    # eval on rank 1
    ...
    for idx, dataset in enumerate(datasets):
        ...
        if test_data is not None:
            ...
            mediaf1mac = mediaf1mac + f1mac
            mediaacc = mediaacc + acc
            if len(datasets)>1 and idx == len(datasets)-1:
                print("Media F1MAC: {0:.3f}".format(mediaf1mac/len(datasets)))
                print("Media ACC: {0:.3f}".format(mediaacc/len(datasets)))
                if epoch==0:
                  maxf1mac = mediaf1mac/len(datasets)
                  maxacc = mediaacc/len(datasets)
                  print("BEST F1MAC EPOCH")
                  print("BEST ACC EPOCH")
                else:
                  if (mediaf1mac/len(datasets))>maxf1mac and test_on==False:
                      maxf1mac = mediaf1mac/len(datasets)
                      print("BEST F1MAC EPOCH")
                  if (mediaacc/len(datasets))>maxacc and test_on==False:
                      maxacc = mediaacc/len(datasets)
                      print("BEST ACC EPOCH")
            if len(datasets) == 1 and test_on==False:
                if epoch==0:
                  maxf1mac = f1mac
                  maxacc = acc
                  print("BEST F1MAC EPOCH")
...
def main():
    ...
    if args.finetuning == True:
        stringa = args.string
        prefix = stringa.split('_')
        for id,i in enumerate(prefix):
            tasks[i]=id
    ...
    train_collater = Collater(dropout_w=args.dropout_w, encoder_type=encoder_type, soft_label=args.mkd_opt > 0, max_seq_len=args.max_seq_len, do_padding=args.do_padding, gan=args.gan)
    ...
    dev_data_list = []
    test_data_list = []
    test_collater = Collater(is_train=False, encoder_type=encoder_type, max_seq_len=args.max_seq_len, do_padding=args.do_padding, gan=args.gan)
    ...
    model = MTDNNModel(opt, device=device, state_dict=state_dict, num_train_step=num_all_batches, gan=gan, apply_scheduler=apply_scheduler, num_hidden_layers_d=args.num_hidden_layers_d, num_hidden_layers_g=args.num_hidden_layers_g, out_dropout_rate=args.dropout_p, noise_size=args.noise_size, epsilon=args.epsilon) ##gan
        ...
        for i, (batch_meta, batch_data) in enumerate(multi_task_train_data):
            ...
            if gan == True:
                ...
                if (model.updates) % (args.log_per_updates) == 0 or model.updates == 1:
                    ...
                    debug_info = ' '
                    print_message(logger, 'Task [{0:2}] Train Generator Loss [{1:.5f}] Train Discriminator Loss [{2:.5f}] {3} remaining [{4}]'.format(task_id,model.train_loss_g.avg,model.train_loss.avg,debug_info,ramaining_time))
            else:
                model.update(batch_meta, batch_data)
                if (model.updates) % (args.log_per_updates) == 0 or model.updates == 1:
                    ramaining_time = str((datetime.now() - start) / (i + 1) * (len(multi_task_train_data) - i - 1)).split('.')[0]
            ...
            if args.save_per_updates_on and ((model.local_updates) % (args.save_per_updates * args.grad_accumulation_step) == 0) and args.local_rank in [-1, 0]:
                ...
                evaluation(model, args.test_datasets, dev_data_list, task_defs, output_dir, epoch, n_updates=args.save_per_updates, with_label=True, tensorboard=tensorboard, glue_format_on=args.glue_format_on, test_on=False, device=device, logger=logger, finetuning = args.finetuning, task = args.task)
                evaluation(model, args.test_datasets, test_data_list, task_defs, output_dir, epoch, n_updates=args.save_per_updates, with_label=False, tensorboard=tensorboard, glue_format_on=args.glue_format_on, test_on=True, device=device, logger=logger, finetuning = args.finetuning, task = args.task)
                print_message(logger, 'Saving mt-dnn model to {}'.format(model_file))
                model.save(model_file)

        evaluation(model, args.test_datasets, dev_data_list, task_defs, output_dir, epoch, with_label=True, tensorboard=tensorboard, glue_format_on=args.glue_format_on, test_on=False, device=device, logger=logger, finetuning = args.finetuning, task = args.task)
        evaluation(model, args.test_datasets, test_data_list, task_defs, output_dir, epoch, with_label=True, tensorboard=tensorboard, glue_format_on=args.glue_format_on, test_on=True, device=device, logger=logger, finetuning = args.finetuning, task = args.task)
