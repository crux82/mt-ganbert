...
class Collater:
    ...
    def collate_fn(self, batch):
        ...
        if self.gan ==  True:
            ...
            label_masks = [sample['label_mask'] for sample in batch]
            task_obj = tasks.get_task_obj(task_def)
            if self.is_train:
                ...
                if task_obj is not None:
                    ...
                    batch_data.append(task_obj.train_prepare_label_mask(label_masks))
                    batch_info['label_mask'] = len(batch_data) - 1 ###dovrebbe essere 3, perché è stato aggiunto in batch_data le labels
    ...
    def _prepare_model_input(self, batch, data_type):
        if self.gan == True:
            ...
            for i, sample in enumerate(batch):
                ...
                masks[i, :select_len] = torch.LongTensor(sample['input_mask'][:select_len])
        else:
            batch_size = self._get_batch_size(batch)
            tok_len = self._get_max_len(batch, key='token_id')
